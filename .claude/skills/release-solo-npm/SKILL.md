...
description: >
  ...
  execute (silent through verification). Composes with /verify.
...
This skill is customized for **npm-trust** (single-package CLI +
library, pnpm + tsc + vitest). Verification commands defer to
`/verify` (the consumer's hand-tuned skill: lint + typecheck + build
+ test + test:e2e + CLI smoke); the inline fallback is `pnpm lint &&
...
to running `pnpm lint && pnpm typecheck && pnpm build && pnpm test &&
pnpm test:e2e` inline.)
...
| `PACKAGE_NOT_PUBLISHED` for `npm-trust` | **STOP** — first-publish ceremony needed; see `/setup-npm-trust` |
...
```markdown
## [v{NEXT}](https://github.com/gagle/npm-trust/compare/v{LATEST}...v{NEXT}) (YYYY-MM-DD)
...
```bash
npm view "npm-trust@${NEXT_VERSION}" version
npm view "npm-trust@${NEXT_VERSION}" dist.attestations
```
...
```bash
npm view "npm-trust@${NEXT_VERSION}" version --registry $REG
```
...
Released v${NEXT_VERSION} to npm.
  Tarball: https://www.npmjs.com/package/npm-trust/v/${NEXT_VERSION}
  CI:      <gh run url>
```
